...
def tst_one(image,spectra,crystal,random_orientation,SIM_WAL=None):
  ...
  run_sim2smv(prefix = file_prefix,crystal = crystal,spectra=iterator,rotation=rand_ori,quick=quick,rank=rank,SIM_WAL=SIM_WAL)
...
if __name__=="__main__":
  ...
  while len(parcels)>0:
    ...
    tst_one(image=idx,spectra=transmitted_info["spectra"],
            crystal=transmitted_info["crystal"],random_orientation=transmitted_info["random_orientations"][idx],SIM_WAL=SIM_WAL)
    parcels.remove(idx)
