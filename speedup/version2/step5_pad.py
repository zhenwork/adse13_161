...
"""
## zhensu: ----------------------------------------------
def SIM_INI(SIM_WAL,N,wavelength_A,UMAT_nm,Amatrix_rot,flux,rank):
  if wavelength_A in SIM_WAL:
    print("### wavelength_A %f already exists in memory"%wavelength_A )
    return SIM_WAL[wavelength_A]
  else:
    print(">>> initialize wavelength_A %f "%wavelength_A )
...
def channel_pixels(wavelength_A,flux,N,UMAT_nm,Amatrix_rot,fmodel_generator,local_data,rank,SIM):
...
def run_sim2smv(prefix,crystal,spectra,rotation,rank,quick=False,save_bragg=False,SIM_WAL=None):
  if SIM_WAL is None:
    SIM_WAL={}
  local_data = data()
  ...
  for x in range(len(flux)):
    ...
    CH_MEM = SIM_INI(SIM_WAL,N,wavlen[x],UMAT_nm,Amatrix_rot,flux[x],rank)
    CH = channel_pixels(wavlen[x],flux[x],N,UMAT_nm,Amatrix_rot,GF,local_data,rank,CH_MEM)
    SIM.raw_pixels += CH.raw_pixels * crystal.domains_per_crystal
    CHDBG_singleton.extract(channel_no=x, data=CH.raw_pixels)
    
    CH_MEM.raw_pixels *= 0 ## zhensu: CH.free_all()
    CH.raw_pixels *= 0 ## zhensu:
    SIM_WAL[wavlen[x]].raw_pixels *= 0 ## zhensu: 
