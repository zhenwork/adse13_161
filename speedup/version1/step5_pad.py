...
## zhensu: ---------------------------------------------------------------
def SIM_INI(N, UMAT_nm, Amatrix_rot):
  SIM = nanoBragg(detpixels_slowfast=(3000,3000), pixel_size_mm=0.11, Ncells_abc=(N,N,N), verbose=0)
...
def channel_pixels(wavelength_A,flux,N,UMAT_nm,Amatrix_rot,fmodel_generator,local_data,rank,SIM): # zhensu: def channel_pixels(wavelength_A,flux,N,UMAT_nm,Amatrix_rot,fmodel_generator,local_data,rank):
  ...
  ## zhensu: cut from here, leave some parts here
  ...
  SIM.Ncells_abc=temp  
  ## zhensu: cut to here, leave some parts here
...
def run_sim2smv(prefix,crystal,spectra,rotation,rank,quick=False,save_bragg=False):
  ...
  CH_IMG = SIM_INI(N, UMAT_nm, Amatrix_rot) ## zhensu: nothing
  for x in range(len(flux)):
    from libtbx.development.timers import Profiler
    if rank==7: P = Profiler("nanoBragg Python and C++ rank %d"%(rank))

    if rank==7: print("+++++++++++++++++++++++++++++++++++++++ Wavelength",x)
    CH = channel_pixels(wavlen[x],flux[x],N,UMAT_nm,Amatrix_rot,GF,local_data,rank,CH_IMG) # zhensu: CH = channel_pixels(wavlen[x],flux[x],N,UMAT_nm,Amatrix_rot,GF,local_data,rank)
    SIM.raw_pixels += CH.raw_pixels * crystal.domains_per_crystal
    CHDBG_singleton.extract(channel_no=x, data=CH.raw_pixels)
    CH.raw_pixels *= 0 # zhensu: CH.free_all()
    CH_IMG.raw_pixels *= 0 # zhensu: nothing

    if rank==7: del P

  CH_IMG.free_all() ## zhensu: nothing
