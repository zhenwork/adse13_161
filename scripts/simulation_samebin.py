...
#################################
simparams.prefix = "lao_samebin"
# simparams.num_pdbs = 2
# simparams.pdb_files = simparams.pdb_files[:2] 
simparams.num_img = [5000] * len(simparams.pdb_files)    
#################################
...
if __name__=="__main__":
    ...
    if rank == 0:
        ...
        """
        ################################################################ 
        mt = flex.mersenne_twister(seed=10)
        random_orientations = []
        import json
        oriData = json.load(open("./orientation_samebin_750000.json"))
        for iteration in range( sum(simparams.num_img) ):
            random_orientations.append( tuple(oriData[iteration]) )
        oriData = None
        ################################################################
        for iteration in range( 150 * 10000 + 10 ): 
            tmpOri = mt.random_double_r3_rotation_matrix()
        tmpOri = None
        ################################################################
